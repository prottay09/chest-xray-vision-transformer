...
def train():
    ...
    model = Cross_ViT(out_size=5)
    #model = DeiT(out_size=5)
    checkpoint_callback = ModelCheckpoint(monitor="val_loss", mode='min', filename = "CrossViT2_w_-100-{epoch:02d}-{val_loss:.2f}", save_last=True)
